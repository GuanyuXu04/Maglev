/*
 * Maglev PD controller -- setup (2): VL53L1X is mounted UNDER the electromagnet
 * and ranges DOWNWARD at the top of the floating magnet, so its reading IS the
 * coil-to-magnet gap directly:
 *
 *      y = coil-to-magnet gap = reading      (no calibration needed)
 *
...
static bool readGap(float *raw_mm, float *y_mm) {
  ...
  float y = (float)mm;                          // ranging down: reading = y directly
...
void setup() {
  ...
  sensor.setDistanceMode(VL53L1X::Short);  // small gap; Short is faster/robust
  sensor.setMeasurementTimingBudget(20000);
  sensor.startContinuous(25);

  // No calibration here: ranging downward, the reading already IS the gap y.
  // Control self-arms -- it only runs while readGap() reports a valid target,
  // so nothing is energized until the magnet is placed within range.
  Serial.println(F("Ranging down (no calibration). Place the magnet to engage."));
  g_lastUs = micros();
}
...
void loop() {
  ...
  if (readGap(&raw, &y)) {          // control runs once per fresh sensor sample
    ...
    // Ranging down, reading and gap_y are the same quantity, so they overlap.
